...
echo "=== Session Start Hook Debug ===" >> "$DEBUG_LOG" 2>&1
echo "Date: $(date)" >> "$DEBUG_LOG" 2>&1
echo "PWD: $PWD" >> "$DEBUG_LOG" 2>&1
echo "CLAUDE_PROJECT_DIR: $CLAUDE_PROJECT_DIR" >> "$DEBUG_LOG" 2>&1
echo "BASH_SOURCE: ${BASH_SOURCE[0]}" >> "$DEBUG_LOG" 2>&1
echo "Shell: $SHELL" >> "$DEBUG_LOG" 2>&1
echo "Bash version: $BASH_VERSION" >> "$DEBUG_LOG" 2>&1
